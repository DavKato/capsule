...
```sh
FORCE_REBUILD=1 ralph 1   # force-rebuild the base image
```

## How it works

1. Builds a base `ralph` image (Arch Linux + git + gh + Claude Code) if not cached
2. Builds a repo-specific `ralph-<repo>` image if `.ralph/Dockerfile` exists
...
Place a `.ralph/` directory in your repo to customise the environment:
...
See `example/.ralph/` for annotated examples of each file.
