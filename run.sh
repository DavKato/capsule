...
CAPSULE_PROMPT="${SCRIPT_DIR}/prompt.md"
...
# If the repo has .capsule/Dockerfile, build a derived image with repo-specific
# tooling (e.g. pnpm, python, postgres-client).
RUN_IMAGE="${IMAGE_NAME}"
if [ -f "$(pwd)/.capsule/Dockerfile" ]; then
	...
	if [[ "${FORCE_REBUILD:-0}" == "1" ]] || ! docker image inspect "${REPO_IMAGE}" &>/dev/null; then
		...
		docker build -t "${REPO_IMAGE}" -f "$(pwd)/.capsule/Dockerfile" "$(pwd)/.capsule"
	fi
...
for ((i = 1; i <= $1; i++)); do
	...
	# Attach to the compose network for this project so Capsule can reach services like postgres.
	...
	# .capsule/.env can override values for the container context (e.g. service hostnames
	...
	if [ -f "$(pwd)/.capsule/.env" ]; then
		ENV_FLAG="${ENV_FLAG} --env-file $(pwd)/.capsule/.env"
	fi

	# Mount optional setup script.
	SETUP_FLAG=""
	if [ -f "$(pwd)/.capsule/setup.sh" ]; then
		SETUP_FLAG="-v $(pwd)/.capsule/setup.sh:/home/claude/setup.sh:ro"
	fi
	...
	if [[ "$result" == *"<promise>NO MORE TASKS</promise>"* ]]; then
		echo ""
		echo "✅ Capsule complete after $i iterations."
		exit 0
	fi
done
